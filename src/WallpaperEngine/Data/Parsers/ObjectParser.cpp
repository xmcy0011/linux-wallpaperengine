...
TextUniquePtr ObjectParser::parseText (const JSON& it, const Project& project, ObjectData base) {
    ...
    // Match parseImage: WE often stores text color as vec3; DynamicValue keeps a parallel m_vec4 with w=0 for
    // Vec3 storage, so getVec4() would yield alpha 0 and CText would draw fully transparent glyphs.
    if (result->color && result->color->value) {
