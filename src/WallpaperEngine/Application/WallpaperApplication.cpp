...
AssetLocatorUniquePtr WallpaperApplication::setupAssetLocator (const std::wstring& bg) const {
    ...
    try {
        ...
        sLog.exception (
            "Cannot find a valid assets folder, resolved to ", this->m_context.settings.general.assets.string ()
        );
    }
...
void WallpaperApplication::advancePlaylist (
    ...
    if (!this->preflightWallpaper (candidatePath)) {
        if (std::find (playlist.failedIndices.begin (), playlist.failedIndices.end (), candidateIndex)
            == playlist.failedIndices.end ()) {
        ...
        if (this->m_renderContext) {
            this->m_renderContext->setWallpaper (
                ...
                WallpaperEngine::Render::CWallpaper::fromWallpaper (
                    *this->m_backgrounds[screen]->wallpaper, *this->m_renderContext, *this->m_audioContext, scaling,
                    clamp
                )
            );
        }
...
void WallpaperApplication::takeScreenshot (const std::filesystem::path& filename) const {
    ...
    const std::string extStr = wstring2string (extension);
    const std::string filenameUtf8 = wstring2string (filename);
...
void WallpaperApplication::setupOutput () {
    ...
    this->m_videoDriver
        = sVideoFactories.createVideoDriver (this->m_context.settings.render.mode, sessionType, this->m_context, *this);
